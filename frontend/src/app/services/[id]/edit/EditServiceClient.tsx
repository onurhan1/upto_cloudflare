...
export default function EditServiceClient() {
    ...
    const loadService = async () => {
        try {
            ...
            if (result.error) {
                ...
                if (service) {
                    form.reset({
                        ...
                        port: service.port ? service.port.toString() : '',
                        check_interval_seconds: service.check_interval_seconds,
                        timeout_ms: service.timeout_ms,
                        expected_status_code: service.expected_status_code ? service.expected_status_code.toString() : '',
                        expected_keyword: service.expected_keyword || '',
                        notify_telegram: service.notify_telegram,
            ...
            toast({
                variant: 'destructive',
                title: 'Error',
                description: err.message || 'Failed to load service',
            });
            router.push('/services');
        } finally {
            setLoading(false);
    ...
    const onSubmit = async (data: ServiceFormValues) => {
        try {
            ...
            const result = await api.updateService(serviceId, serviceData);
            ...
            <div className="max-w-3xl space-y-6">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Update monitoring settings for your service
                    </p>
                </div>
